...
export const NODE_DEFINITIONS: Record<string, NodeDefinition> = {
  ...
  'action.wait_for_reply': {
    type: 'action.wait_for_reply',
    kind: 'action',
    label: 'Aguardar resposta do aluno',
    description: 'Pausa o fluxo e continua por uma branch dependendo se o aluno responder (qualquer mensagem) dentro do prazo ou não.',
    icon: MessageCircleReply,
    color: 'amber',
    defaultData: { wait_days: 3 },
    fields: [
      {
        key: 'wait_days',
        type: 'number',
        label: 'Dias para aguardar',
        min: 1,
        max: 30,
        step: 1,
        required: true,
        helpText: 'Se o aluno não responder em até esse prazo, segue pela branch "Não respondeu".',
      },
    ],
  },
