...
const statusConfig: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  draft: { label: 'Rascunho', color: 'text-gray-700', bg: 'bg-gray-100', icon: Clock },
  ...
  cancelled: { label: 'Cancelado', color: 'text-gray-500', bg: 'bg-gray-100', icon: XCircle },
};
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className={`transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-br from-[#2A658F] to-[#3d7ba8]">
                <Send className="w-6 h-6 text-white" />
              </div>
              <div>
                <h1 className="text-2xl font-semibold text-[#27273D]">{broadcast.name}</h1>
                {broadcast.description && <p className="text-sm text-gray-500">{broadcast.description}</p>}
              </div>
            </div>

            <div className="flex items-center gap-2">
              {broadcast.status === 'sending' && (
                <button onClick={loadBroadcast} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-[#2A658F] bg-[#E2ECF4] rounded-xl hover:bg-[#CCE4F4] transition-all">
        ...
        <div className={`grid grid-cols-2 lg:grid-cols-5 gap-4 transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: '100ms' }}>
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-100">
            <p className="text-2xl font-semibold text-[#27273D]">{broadcast.total_students}</p>
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: '200ms' }}>
          <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between py-2 border-b border-gray-50">
                ...
                <span className="text-sm text-gray-700">{formatDate(broadcast.created_at)}</span>
              </div>
              {broadcast.started_at && (
                <div className="flex justify-between py-2 border-b border-gray-50">
                  <span className="text-sm text-gray-500">Iniciado em</span>
                  <span className="text-sm text-gray-700">{formatDate(broadcast.started_at)}</span>
                </div>
              )}
              {broadcast.completed_at && (
                <div className="flex justify-between py-2">
                  <span className="text-sm text-gray-500">Finalizado em</span>
                  <span className="text-sm text-gray-700">{formatDate(broadcast.completed_at)}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <h2 className="text-lg font-semibold text-[#27273D] mb-3">Filtros aplicados</h2>
            {Object.keys(broadcast.filters || {}).length === 0 ? (
              <p className="text-sm text-gray-400 italic">Sem filtros (todos os alunos com telefone)</p>
            ) : (
                ...
                <div className="space-y-1">
                  {broadcast.template_params.map((p: string, i: number) => (
                    <p key={i} className="text-sm text-gray-600">
                      <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">{`{{${i + 1}}}`}</code> → {p}
                    </p>
                  ))}
                </div>
              </div>
        ...
        {/* Logs */}
        <div className={`bg-white rounded-2xl border border-gray-100 transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: '300ms' }}>
          ...
          {!broadcast.logs || broadcast.logs.length === 0 ? (
            <div className="text-center py-12">
              ...
              {broadcast.status === 'draft' && (
                <p className="text-xs text-gray-400 mt-1">Os logs aparecerão após iniciar o envio</p>
              )}
            </div>
          ) : (
                  ...
                  {broadcast.logs.map((log) => {
                    ...
                    return (
                      <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-3 text-sm font-medium text-gray-800">{log.student_name}</td>
                        <td className="px-6 py-3 text-sm text-gray-600">{log.phone}</td>
