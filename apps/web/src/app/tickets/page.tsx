...
import { useRouter } from 'next/navigation'; // Adicionado para navegação via linha
...
// Melhoria: SLA agora considera o status do ticket
function getSlaStatus(slaDeadline: string, status: string): { label: string; color: string } {
  // Se já foi resolvido ou fechado, o SLA não é mais "urgente" visualmente
  ...
  if (hoursLeft < 0) return { label: 'Atrasado', color: 'bg-red-500 text-white font-bold' }; // Texto mais alarmante
  ...
  // Se for muito tempo, mostra em dias para facilitar leitura
  const daysLeft = Math.round(hoursLeft / 24);
  return { label: `${daysLeft} dias`, color: 'bg-green-100 text-green-800' };
}

// Formatador de data simples para Tooltips
...
export default function TicketsPage() {
  ...
  const [searchTerm, setSearchTerm] = useState<string>(''); // Novo filtro de busca
  
  const [loadError, setLoadError] = useState(false);
  const [assigningId, setAssigningId] = useState<number | null>(null); // Loading state específico para ação
  ...
  // Filtragem no cliente para a busca de texto (assumindo que a API não tem search endpoint ainda)
  // Se a API tiver, mova isso para o fetchTickets
  ...
  const handleAssign = async (e: React.MouseEvent, ticketId: number) => {
    e.stopPropagation(); // Impede que o clique no botão abra a linha (navegação)
    setAssigningId(ticketId);
    try {
      await api.patch(`/tickets/${ticketId}/assign`);
      // Atualiza localmente para feedback instantâneo antes do fetch
      setTickets(prev => prev.map(t => 
        t.id === ticketId ? { ...t, assigned_to_id: 999, assigned_to: { id: 999, name: 'Você' } } : t
      ));
      await fetchTickets(); // Sincroniza dados reais
    } catch (err) {
      console.error('Erro ao assumir ticket:', err);
      alert('Não foi possível assumir o ticket.');
    } finally {
      setAssigningId(null);
    }
  };
  ...
  return (
    <AppLayout>
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Botão de Atualizar Manual (Melhoria de UX para dados em tempo real) */}
        <button 
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm mb-6 flex flex-col lg:flex-row gap-4 lg:items-end">
        ...
        <div className="flex-1">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="Protocolo, nome do aluno ou assunto..."
              value={searchTerm}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredTickets.map((ticket) => {
                  ...
                  const isAssignedToMe = false; // Lógica para destacar meus tickets (opcional se tiver user context)
